...
declare global {
  interface Window {
    measurementTransport: {
      ...
      requests: number;
      workers: number;
      analysisRequests: boolean[];
      analysisResponses: boolean[];
    };
  }
}

test('worker settings preserve the previous view and apply only the latest request', async ({
  page
}) => {
  await page.addInitScript(() => {
    const OriginalWorker = window.Worker;
    window.measurementTransport = {
      hold: true,
      requests: 0,
      workers: 0,
      analysisRequests: [],
    ...
    window.Worker = class extends OriginalWorker {
      constructor(url: string | URL, options?: WorkerOptions) {
        ...
        if (String(url).includes('turbopack-worker'))
          window.measurementTransport.workers += 1;
        this.addEventListener('message', (event) => {
          if (event.data.type === 'view' && event.data.ok) {
            window.measurementTransport.analysisResponses.push(
              Boolean(event.data.view.analysis)
            );
          }
        });
      }
      postMessage(
        ...
        if (
          ...
          if (
            ...
            'includeAnalysis' in message.settings
          ) {
            window.measurementTransport.analysisRequests.push(
              message.settings.includeAnalysis !== false
            );
          }
  ...
  expect(await page.evaluate(() => window.measurementTransport.requests)).toBe(
    1
  );
  await page.evaluate(() => {
    window.measurementTransport.release?.();
  ...
  await page.evaluate(() => {
    window.measurementTransport.hold = true;
  });
  await page
    .getByRole('combobox', { name: 'Display units' })
    .selectOption('imperial');
  await expect(
    page.getByRole('status', { name: 'Updating measurements' })
  ).toHaveText('Updating…');
  await expect(selected).toContainText('66.7 km/h');
  await page
  ...
  expect(
    await page.evaluate(() => window.measurementTransport.analysisRequests)
  ).toEqual([true, true, false, false]);
  expect(
    await page.evaluate(() => window.measurementTransport.analysisResponses)
  ).toEqual([true, true, false, false]);
  expect(await page.evaluate(() => window.measurementTransport.workers)).toBe(
    1
  );
