...
for (const colorScheme of ['light', 'dark'] as const) {
  test(`public pages and viewer states have no automated accessibility violations in ${colorScheme} mode`, async ({
    page
  }, testInfo) => {
    ...
    await picker.setInputFiles({
      name: 'accessibility.gpx',
      mimeType: 'application/gpx+xml',
      buffer: Buffer.from(
        '<gpx version="1.1"><trk><name>Accessibility route</name><trkseg><trkpt lat="0" lon="0"><ele>10</ele><time>2026-01-01T00:00:00Z</time></trkpt><trkpt lat="0" lon="0.01"><ele>20</ele><time>2026-01-01T00:01:00Z</time></trkpt></trkseg></trk></gpx>'
      )
    });
    ...
    await scan('selected');
    await page.screenshot({
      path: testInfo.outputPath(`selected-${colorScheme}.png`)
    });
