...
const recording = (
  pointCount: number,
  segmentLength = pointCount,
  extensions = false
) => {
  const points = Array.from({ length: pointCount }, (_, index) => {
    const startsSegment = index > 0 && index % segmentLength === 0;
    const missing =
      segmentLength !== pointCount && index % segmentLength === 10;
  ...
  return {
    ...
    buffer: Buffer.from(
      `<gpx xmlns="http://www.topografix.com/GPX/1/1" xmlns:x="urn:gpsgoblin:uninterpreted-test-fields" version="1.1"><trk><name>Synthetic long recording</name><trkseg>${points}</trkseg></trk></gpx>`
    )
  };
};

test('all-day and multi-day recordings retain every sample and recover from failed replacement', async ({
  page
}, testInfo) => {
  ...
  for (const pointCount of [43201, 172801]) {
    ...
    await page.screenshot({
      path: testInfo.outputPath(`${pointCount}-loaded.png`)
    });
    ...
    await expect(
      ...
      pointCount === 43201
        ? '1 January 2026 at 12:00:00 UTC'
        : '3 January 2026 at 00:00:00 UTC'
    );
  }
  await page
  ...
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('14:59 min/km');
  await picker.setInputFiles({
    name: 'truncated.gpx',
    mimeType: 'application/gpx+xml',
    buffer: Buffer.from('<gpx version="1.1"><trk>')
  });
  await expect(
    page.getByText(
...
test('extension-heavy input stays local without interpreting vendor measurements', async ({
  page,
  baseURL
}, testInfo) => {
  ...
  await page.screenshot({
    path: testInfo.outputPath('extensions-selected.png')
  });
});

test('thousands of segments preserve chart gaps and missing measurements', async ({
  page
}, testInfo) => {
  test.setTimeout(90000);
  await page.goto('/tools/gpx-file-viewer.html');
  ...
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('Recorded time: Missing');
  await position.press('ArrowRight');
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('100.0 m');
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('00:00:11 UTC');
  await position.press('End');
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('13:53:19 UTC');
  await page.screenshot({
    path: testInfo.outputPath('fragmented-selected.png')
  });
  expect(
    await page.evaluate(
      () => document.documentElement.scrollWidth <= innerWidth
