...
for (const viewport of [
  ...
  test(`custom pace range clips the view and keeps overflow selectable at ${viewport.width} × ${viewport.height}`, async ({ browser }, testInfo) => {
    ...
    await expect(page.getByText(/Custom range ·/)).toHaveText((suggestedLabel ?? '').replace('Suggested range', 'Custom range'));
    ...
    await expect(page.getByText('Enter a number greater than zero. Showing the full range.')).toBeVisible();
    await expect(page.getByRole('button', { name: /Above range:/ })).toHaveCount(0);
