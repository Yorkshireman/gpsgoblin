...
const recording = (gapCount = 3) => {
  let seconds = 0;
  const intervals = [0, ...Array(20).fill(10), ...Array(gapCount).fill(600), ...Array(20).fill(10)];
  return `<gpx version="1.1" creator="Synthetic gap test" xmlns="http://www.topografix.com/GPX/1/1"><trk><trkseg>${intervals.map((duration, index) => {
    seconds += duration;
    const longitude = index > 20 && index < 21 + gapCount ? 0.002 : index < 21 + gapCount ? index / 10000 : (index - gapCount) / 10000;
...
for (const viewport of [{ width: 1440, height: 900 }, { width: 1280, height: 720 }, { width: 390, height: 844 }, { width: 375, height: 667 }]) {
  test(`gap inspection ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    await page.setViewportSize(viewport);
    await page.goto('/tools/gpx-file-viewer.html');
    await page.getByLabel('GPX file', { exact: true }).setInputFiles({ name: 'recording-gaps.gpx', mimeType: 'application/gpx+xml', buffer: Buffer.from(recording()) });
    ...
    await paceTip.focus();
    await page.keyboard.press('Enter');
    await expect(page.getByText('A higher number means a slower pace.', { exact: false })).toBeVisible();
    await expect(page.getByText('Pace is the time it takes to cover one mile.', { exact: false })).toBeVisible();
    await page.screenshot({ path: testInfo.outputPath('pace-tip.png') });
    await page.keyboard.press('Enter');
    await expect(page.getByText('A higher number means a slower pace.', { exact: false })).toBeHidden();
...
test('permissioned local recordings preserve gap distinction', async ({ page }, testInfo) => {
  const gapFile = process.env.GPSGOBLIN_GAP_FILE;
  const continuousFile = process.env.GPSGOBLIN_CONTINUOUS_FILE;
  test.skip(!gapFile || !continuousFile, 'Local private recordings supplied only for owner validation');
  ...
  await page.screenshot({ path: testInfo.outputPath('private-continuous-recording.png') });
});
