...
const file = (
  name: string,
  contents = '<gpx version="1.1"><wpt lat="0" lon="0"><name>Safe waypoint</name></wpt></gpx>'
) => {
  return {
    name,
...
test('actual worker rejects hostile input without network requests and recovers', async ({
  page,
  baseURL
}) => {
  ...
  for (const contents of [
    ...
    await expect(
      page.getByText('Safe waypoint', { exact: true })
    ).toBeVisible();
    await expect(
      page.getByText('valid.gpx', { exact: true }).first()
    ).toBeVisible();
  }
  const markup =
    '<img src="https://gpx-canary.invalid/pixel" onerror="alert(1)">';
  await picker.setInputFiles(
    file(
      'markup.gpx',
      `<gpx version="1.1"><wpt lat="0" lon="0"><name><![CDATA[${markup}]]></name><link href="https://gpx-canary.invalid/link"/></wpt></gpx>`
    )
  );
  ...
  await expect(
    page.getByRole('button', { name: 'Choose GPX file', exact: true })
  ).toBeVisible();
  await picker.setInputFiles(file('valid.gpx'));
  await expect(page.getByText('Safe waypoint', { exact: true })).toBeVisible();
});
...
for (const viewport of [
  ...
  test(`cancel, clear and retry remain usable at ${viewport.width} × ${viewport.height}`, async ({
    browser
  }, testInfo) => {
    ...
    await picker.setInputFiles(file('replacement.gpx'));
    await expect(
      page.getByText('replacement.gpx', { exact: true }).first()
    ).toBeVisible();
    ...
    await page.screenshot({
      path: testInfo.outputPath('failed-replacement.png')
    });
    expect(
      await page.evaluate(
...
test('a long-running import remains cancellable and preserves the previous file', async ({
  page
}) => {
  await page.goto('/tools/gpx-file-viewer.html');
  const picker = page.getByLabel('GPX file', { exact: true });
  ...
  try {
    ...
    await expect(
      page.getByText('original.gpx', { exact: true }).first()
    ).toBeVisible();
    await expect(
      page.getByText('Safe waypoint', { exact: true })
    ).toBeVisible();
  ...
  await picker.setInputFiles(file('slow.gpx'));
  await expect(
    page.getByText('slow.gpx', { exact: true }).first()
  ).toBeVisible();
});

test('rejects multiple dropped files and permits a single-file recovery', async ({
  page
}) => {
  await page.goto('/tools/gpx-file-viewer.html');
  ...
  await page
    ...
    .setInputFiles(file('one.gpx'));
  await expect(page.getByText('Safe waypoint', { exact: true })).toBeVisible();
});

test('clearing a pending import prevents it from restoring data after a newer import', async ({
  page
}) => {
  await page.goto('/tools/gpx-file-viewer.html');
  const picker = page.getByLabel('GPX file', { exact: true });
  ...
  try {
    ...
    await expect(page.getByText('Safe waypoint', { exact: true })).toHaveCount(
      0
    );
    await picker.setInputFiles(file('new.gpx'));
    await expect(
      page.getByText('new.gpx', { exact: true }).first()
    ).toBeVisible();
  } finally {
    release?.();
    await page.unrouteAll({ behavior: 'wait' });
...
test('first-import progress and failure feedback fit the short phone viewport', async ({
  browser
}, testInfo) => {
  const context = await browser.newContext({
  ...
  try {
    await page
      .getByLabel('GPX file', { exact: true })
      .setInputFiles(file('first.gpx'));
    const cancel = page.getByRole('button', { name: 'Cancel import' });
    await expect(cancel).toBeInViewport({ ratio: 1 });
    await page.screenshot({
      path: testInfo.outputPath('first-processing.png')
    });
    await cancel.tap();
  } finally {
    release?.();
  ...
  await page
    .getByLabel('GPX file', { exact: true })
    .setInputFiles(file('bad.gpx', '<gpx>'));
  await expect(
    page.getByText(
      'The file is damaged or incomplete. Download a new copy and try again.'
    )
...
test('the actual worker imports the complete sanitised Strava recording', async ({
  page
}) => {
  await page.goto('/tools/gpx-file-viewer.html');
