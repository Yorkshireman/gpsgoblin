...
const points = Array.from({ length: 50000 }, (_, index) => {
  const longitude =
    (index < 9000 ? index : index <= 9100 ? 8999 : index - 101) * 0.00001;
  const elevation = index === 3457 ? 900 : index === 4567 ? -50 : 100;
  const missing =
    (index >= 6000 && index < 6500) ||
    (index >= 12500 && index < 12600 && index !== 12553);
...
test('chart detail adapts to width while every original measurement stays selectable', async ({
  ...
  expect(
    vertices(widePath).some((point) => Math.abs(point.x - selectedX) < 0.0001)
  ).toBe(false);
  await expect(trace).toHaveAttribute('d', widePath);
  await expect(
    page.getByRole('img', {
      name: 'Selected map position: 0, 0.00002',
      exact: true
    })
  ).toBeVisible();
  await page.screenshot({
    path: testInfo.outputPath('large-selected-desktop.png')
  });
  await page.setViewportSize({ width: 375, height: 667 });
  await expect
    .poll(async () => vertices((await trace.getAttribute('d')) ?? '').length)
    .toBeLessThan(vertices(widePath).length);
  ...
  await expect(
    page.getByRole('img', {
      name: 'Selected map position: 0, 0.49898',
      exact: true
    })
  ).toBeVisible();
  ...
  await page.screenshot({
    path: testInfo.outputPath('large-selected-phone.png')
  });
});

test('a dense chart preserves sharp extrema, isolated readings and stop transitions for touch selection', async ({
  browser
}, testInfo) => {
  ...
  const tapVertex = async (highest: boolean) => {
    ...
    const screen = await trace.evaluate((element, point) => {
      if (!(element instanceof SVGPathElement))
        throw new Error('Missing chart trace');
  ...
  await expect(
    page.getByRole('img', {
      name: 'Selected map position: 0, 0.08999',
      exact: true
    })
  ).toBeVisible();
...
test('separate fragments retain their own peaks even inside the same screen pixel', async ({
  page
}) => {
