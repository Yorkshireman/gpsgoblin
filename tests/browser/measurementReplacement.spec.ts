...
const recording = (day: number, elevation: number) => {
  return {
    ...
    buffer:
      Buffer.from(`<gpx version="1.1"><trk><name>Recorded track</name><trkseg>
...
test('successful replacement resets the previous selection and file-owned chart settings', async ({
  page
}) => {
  ...
  await expect(
    page.getByLabel('Chart selection tip', { exact: true })
  ).toBeVisible();
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toHaveCount(0);
  ...
  await expect(page.getByRole('slider', { name: 'Smoothing' })).toHaveAttribute(
    'aria-valuetext',
    '1 minute'
  );
  await chart.selectOption('pace');
  ...
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('220.0 m');
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toContainText('2 January 2026 at 00:02:00 UTC');
});

test('replacement after a real worker view error clears old failure and starts a usable session', async ({
  page
}) => {
  await page.addInitScript(() => {
    ...
    window.Worker = class extends OriginalWorker {
      postMessage(
        ...
        if (
          window.rejectNextMeasurementView &&
          message &&
          ...
          message = {
            ...message,
            settings: { ...message.settings, entityId: 'unavailable-entity' }
          };
        }
  ...
  await position.press('End');
  await page.evaluate(() => {
    window.rejectNextMeasurementView = true;
  });
  await units.selectOption('imperial');
  await expect(
    page.getByText(
