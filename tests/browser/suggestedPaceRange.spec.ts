...
const recording = () => {
  let longitude = 0;
  return `<gpx version="1.1" xmlns="http://www.topografix.com/GPX/1/1"><trk><trkseg>${Array.from(
    { length: 1200 },
    (_, index) => {
...
for (const viewport of [
  ...
  test(`suggested pace is inspectable at ${viewport.width}x${viewport.height}`, async ({
    ...
    await expect(
      page.getByText(/Some pace readings are above the chart limit/)
    ).toBeVisible();
    await expect(
      page.getByRole('combobox', { name: 'Pace range' })
    ).toBeHidden();
    ...
    await page
      .getByLabel('Measurement chart', { exact: true })
      .evaluate((element) => {
    ...
    await page.screenshot({
      path: testInfo.outputPath('selected-overflow.png')
    });
    ...
    await expect(page.getByText(/Custom range ·/)).toContainText(
      '46:40 min/mi'
    );
    await page.screenshot({
      path: testInfo.outputPath('prefilled-custom.png')
    });
...
test('permissioned exports receive different suggested ranges', async ({
  page
}, testInfo) => {
  const files = [
    process.env.GPSGOBLIN_GAP_FILE,
    process.env.GPSGOBLIN_CONTINUOUS_FILE
  ];
  test.skip(
    files.some((file) => {
      return !file;
    }),
    'Requires local private recordings'
  );
  for (const [index, file] of files.entries()) {
    ...
    await page
      .getByRole('combobox', { name: 'Chart', exact: true })
      .selectOption('pace');
    await expect(page.getByText(/Suggested range ·/)).toContainText(
      index === 0 ? '54:00 min/km' : '32:00 min/km'
    );
    await expect(
      page.getByText(/Some pace readings are above the chart limit/)
    ).toBeVisible();
    if (index === 0) {
      const gaps = await page
        .getByRole('button', { name: /Recording gap|nearby recording gaps/ })
        .all();
      ...
      for (const gap of gaps) {
        ...
        for (const arrow of arrows) {
          ...
          if (gapBox && arrowBox)
            expect(gapBox.y + gapBox.height).toBeLessThanOrEqual(arrowBox.y);
        }
      }
    }
    await page.screenshot({
      path: testInfo.outputPath(`private-pace-${index}.png`)
    });
  }
});
