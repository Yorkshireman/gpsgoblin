...
for (const paths of [
  ...
  test(`pages hydrate after visiting ${paths.join(' then ')}`, async ({ page }) => {
    const errors: string[] = [];
    page.on('console', message => {
      if (message.type() === 'error' && /hydrat|server rendered|react\.dev\/errors\/(418|419|421|422|423|425)/i.test(message.text())) {
