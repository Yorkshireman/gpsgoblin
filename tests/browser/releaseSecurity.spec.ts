...
test('exported pages enforce headers and hydrate on direct clean URLs', async ({
  page
}) => {
  const errors: string[] = [];
  ...
  page.on('console', (message) => {
    if (/Content Security Policy|hydration|Hydration/i.test(message.text()))
  ...
  const blocked = await page.evaluate(async () => {
    return await new Promise<string>((resolve) => {
      ...
      script.textContent =
        "document.documentElement.dataset.unapprovedScript = 'executed'";
      document.head.append(script);
    });
  });
  expect(blocked).toBe('script-src-elem');
  await expect(page.locator('html')).not.toHaveAttribute(
    'data-unapproved-script'
  );
});

test('activity canaries stay local through import, inspection, replacement and clear', async ({
  page,
  context,
  baseURL
}) => {
  const canaries = [
    'release-private-file',
    'ReleasePrivateLabel',
    '53.1234567',
    '-1.7654321',
    '2031-07-19T09:37:41Z',
    '187.654',
    '123.456'
  ];
  const requests: {
  ...
  await picker.setInputFiles({
    ...
    buffer:
      Buffer.from(`<gpx version="1.1" xmlns:gpxtpx="http://www.garmin.com/xmlschemas/TrackPointExtension/v1"><trk><name>${canaries[1]}</name><trkseg>
  ...
  await expect(
    page.getByRole('img', { name: /Selected map position: 53.123/ })
  ).toBeVisible();
  await expect(
  ...
  await page
    .getByRole('button', { name: 'Close point details' })
    .press('Enter');
  await expect(
    page.getByLabel('Selected measurement', { exact: true })
  ).toHaveCount(0);
  await picker.setInputFiles({
    ...
    mimeType: 'application/gpx+xml',
    buffer: Buffer.from(
      '<gpx version="1.1"><wpt lat="0" lon="0"><name>Replacement waypoint</name></wpt></gpx>'
    )
  });
  await expect(
    page.getByText('Replacement waypoint', { exact: true })
  ).toBeVisible();
  ...
  expect(
    requests.some(({ url }) => {
      return url.includes('/maplibre/maplibre-gl-worker.mjs');
    })
  ).toBe(true);
  expect(
    requests.some(({ url }) => {
      return url.includes('/turbopack-worker-');
    })
  ).toBe(true);
  expect(
    requests.some(({ url }) => {
      return url.startsWith('https://tile.openstreetmap.org/');
    })
  ).toBe(true);
  for (const request of requests) {
    ...
    expect(
      url.origin === baseURL ||
        /^https:\/\/tile\.openstreetmap\.org\/\d+\/\d+\/\d+\.png$/.test(
          request.url
        )
    ).toBe(true);
    ...
    for (const canary of canaries)
      expect(decodeURIComponent(JSON.stringify(request))).not.toContain(canary);
  }
...
test('Workers verification hosts are noindex without affecting the public origin', async ({
  request
}) => {
  for (const host of [
    'gpsgoblin.example.workers.dev',
    'version-gpsgoblin.example.workers.dev',
    'branch-gpsgoblin.example.workers.dev',
    'gpsgoblin.com'
  ]) {
    const response = await request.get('/tools/gpx-file-viewer', {
      headers: { Host: host }
    });
    expect(response.status()).toBe(200);
    expect(response.headers()['x-robots-tag']).toBe(
      host.endsWith('.workers.dev') ? 'noindex' : undefined
    );
...
test('static assets exclude private files and known credential signatures', async () => {
  ...
  for (const file of files) {
    ...
    if (/\.(?:html|js|mjs|css|json|txt|xml|map)$/.test(file)) {
      ...
      expect(
        credential.test(readFileSync(path.join('out', file), 'utf8')),
        `Credential signature in ${file}`
      ).toBe(false);
    }
  }
});
