...
2. **The bounded two-page GPX help pilot shipped and was publicly verified** through issue #58 and PR #59, the strongest commercial next action selected through the mandatory “what next?” workflow in `AGENTS.md` and section 23. Commercial assessment remains separate from technical release.
...
### 4.6 GPX value and manual file acquisition — agreed direction
...
Unsupported, ambiguous or malformed files need understandable recovery guidance, not a guessed viewer, silent conversion or false claim that every activity format is supported. Test routing for every released format, misleading extensions, invalid files and file/data preservation through navigation before release.
...
The default viewer keeps introductory copy brief and puts longer instructions under **Help with this viewer**. Show **Calculated distance** and **Elapsed time**, with their explanations available under **About these totals**. Keep missing-time messages, planned-route uncertainty, warning disclosures and an active filtered chart's basis discoverable. File/session details and map privacy information can be expanded on demand. On phones, the map's **Back to chart** action stays visible when its contents need scrolling.
...
Keep the initial graph understandable without opening settings. Place advanced stop-exclusion and axis controls in a plainly named disclosure near the chart, called **Advanced Controls**, rather than exposing every choice initially. Keep gap markers and essential summary information visible in the default view. When a filtered view is active, show its calculation basis beside the graph with the options closed. The disclosure uses a subtle neutral background, border, sliders icon and chevron. Put **Review possible stops** inside it. Show the Stops selector and stop review only when the selected track or section has possible stops; otherwise omit both controls and empty stop explanations. Put the recording-gap inspector inside Advanced Controls as well, while keeping chart gap markers and active calculation explanations visible independently. Retain the complete-recording calculation when there are no stop candidates.

Use conservative stop detection that retains ambiguous slow movement, including uphill or near-vertical climbing. The agreed first viewer implementation identifies possible stops using the [documented confinement and observation policy](stop-detection-policy.md#agreed-viewer-implementation). Every candidate stays included until individually confirmed by the user; there is no automatic bulk exclusion. Validation did not support a moving-only default, so Include stops remains the initial view. Keep detected stops, device timer pauses and unknown recording gaps distinct. In the moving-only view, exclude unknown gaps from both eligible distance and time for moving-speed calculations, disclose their duration separately from detected stops, and retain visible breaks. Do not imply that unknown time was stationary or that a partial observation establishes the whole activity's moving time.
...
The owner approved bringing the #13 viewer feature forward on 14 September 2026 and agreed the conservative policy after validation. This does not make stop detection a Stage 1 dependency or bring the comparison/replay engine forward. The viewer's candidate and confirmation policy is settled for this iteration; O9 remains open for automatic detection, broader calibration and comparison eligibility. The viewer choices do not change the comparison-specific exclusion rules in section 11. The separate Moving time summary UI remains owned by #11; #13 supplies its shared interval meanings and derived values.
...
The pace axis defaults to a Suggested range using the distance-weighted policy in [pace-range policy](pace-range-policy.md). Keep a direct Show full range action visible and place Suggested / Full range / Custom maximum settings under Advanced Controls. State when slower values exceed the visible range; a short genuine slow section may be clipped and must remain inspectable. Users may explicitly choose a custom axis maximum in the current pace units. This changes only the viewport: clip the line beyond that range without flattening its values, identify overflow visibly, and keep the actual measurements selectable with an above-range explanation. Convert the chosen maximum when display units change. This control does not detect stops, exclude time or change source data or calculations.
