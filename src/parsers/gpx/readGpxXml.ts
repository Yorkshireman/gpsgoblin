...
const textField = (name: string): keyof TextFields | undefined => {
  if (name === 'name' || name === 'desc' || name === 'ele' || name === 'time')
    return name;
  return undefined;
};
...
export const readGpxXml = (fileText: string): GpxParseResult => {
  ...
  const openElement = (tag: SaxesTagNS): Frame => {
    ...
    if (name === 'trkpt') {
      return parent?.kind === 'segment'
        ? pointFrame(
            tag,
            'track point',
            `${parent.id}-sample-${parent.samples.length}`,
            parent.samples
          )
    ...
    if (parent?.kind === 'track' && name === 'trkseg') {
      return {
        kind: 'segment',
        id: `${parent.id}-segment-${parent.segments.length}`,
        samples: [],
        target: parent.segments
      };
    }
    if (parent?.kind === 'route' && name === 'rtept') {
      return pointFrame(
        tag,
        'route point',
        `${parent.id}-point-${parent.points.length}`,
        parent.points
      );
    }
    ...
    if (
      ...
      field &&
      supportedText &&
      !(field in parent.fields)
    ) {
      parent.fields[field] = '';
      const frame: Extract<Frame, { kind: 'text' }> = {
        kind: 'text',
  ...
  parser.on('closetag', () => {
    ...
    if (frame?.kind === 'point') {
      const point = readGpxPoint(
        frame,
        frame.pointKind !== 'track point',
        issues[frame.pointKind]
      );
      frame.target?.push(point);
    }
    if (frame?.kind === 'segment')
      frame.target.push({ id: frame.id, samples: frame.samples });
    if (frame?.kind === 'track')
      tracks.push({
        id: frame.id,
        ...readNames(frame.fields),
