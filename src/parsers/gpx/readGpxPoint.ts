...
export const readGpxPoint = (
  ...
  if (
    elevationText !== undefined &&
    (!elevationText.trim() || !Number.isFinite(elevationMetres))
  ) {
  ...
  return {
    ...
    ...(point.fields.time === undefined
      ? {}
      : { sourceTime: point.fields.time }),
...
export const pointError = (issues: PointIssues, kind: PointKind) => {
  if (issues.missingCoordinates)
    return `A ${kind} is missing its map location. Download a new copy and try again.`;
  if (issues.invalidCoordinates)
    return `A ${kind} has a map location that cannot be read. Download a new copy and try again.`;
  if (issues.invalidElevation)
    return `A ${kind} has a height reading that cannot be read. Download a new copy and try again.`;
  return undefined;
};
