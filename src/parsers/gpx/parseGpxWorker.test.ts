...
it('parses GPX without window or the browser DOM, as required in a worker', () => {
  const result = parseGpx(
    '<gpx version="1.1"><wpt lat="0" lon="0"><name>Zero</name></wpt></gpx>'
  );
  expect(result.ok).toBe(true);
  if (!result.ok) throw new Error(result.error);
  expect(result.document.waypoints[0]).toMatchObject({
    name: 'Zero',
    latitudeDegrees: 0,
    longitudeDegrees: 0
  });
...
it('reads the sanitised bikerouter exporter fixture while preserving its source structure', () => {
  const contents = readFileSync(
    'tests/fixtures/gpx/bikerouter-2025.46.gpx',
    'utf8'
  );
  const result = parseGpx(contents);
  expect(result.ok).toBe(true);
...
it('explicitly rejects the sanitised GPSBabel GPX 1.0 recording fixture', () => {
  expect(
    parseGpx(readFileSync('tests/fixtures/gpx/gpsbabel-gpx10.gpx', 'utf8'))
  ).toEqual({
    ok: false,
    error:
...
it.each([
  ...
  'rejects hostile or malformed XML without producing a document: %s',
  (contents) => {
    expect(parseGpx(contents).ok).toBe(false);
  }
);

it.each(['a & b', '\u0000', '&#0;', '&#xD800;', '<![CDATA[bad\u0000]]>'])(
  'rejects invalid XML character data: %s',
  (value) => {
    const result = parseGpx(
      `<gpx version="1.1"><wpt lat="0" lon="0"><name>${value}</name></wpt></gpx>`
    );
...
it('imports every point in the permissioned, sanitised Strava recording', () => {
  const contents = readFileSync(
    'tests/fixtures/gpx/strava-recording.gpx',
    'utf8'
  );
  const result = parseGpx(contents);
  expect(result.ok).toBe(true);
  ...
  expect(samples[0]).toMatchObject({
    ...
    longitudeDegrees: 0,
    elevationMetres: 101,
    sourceTime: '2026-01-01T00:00:01Z'
  });
...
it('preserves valid escaped and CDATA text while rejecting malformed XML', () => {
  const result = parseGpx(
    '<gpx version="1.1"><wpt lat="0" lon="0"><name>A &amp; B &#x1F600; <![CDATA[<C & D>]]></name></wpt></gpx>'
  );
  expect(result.ok).toBe(true);
  if (!result.ok) throw new Error(result.error);
            ...
            samples: [
              {
                ...
                longitudeDegrees: 0,
                elevationMetres: 0,
                sourceTime: ' 2026-09-12T01:02:03Z '
              }
            ]
          },
          {
            id: 'track-0-segment-2',
            samples: [
              {
                id: 'track-0-segment-2-sample-0',
                latitudeDegrees: 1,
                longitudeDegrees: 2,
                sourceTime: ''
              }
...
it('validates XML in ignored extensions and after supported geographic content', () => {
  ...
  expect(parseGpx(contents)).toEqual({
    ok: false,
    error:
      'The file is damaged or incomplete. Download a new copy and try again.'
  });
...
it('reports missing coordinates before invalid values independently of source order', () => {
  const contents =
    '<gpx version="1.1"><wpt lat="0"/><trk><trkseg><trkpt lat="0"/></trkseg></trk><rte><rtept lat="invalid" lon="0"/><rtept lat="0" lon="0"><ele>invalid</ele></rtept><rtept lat="0"/></rte></gpx>';
  expect(parseGpx(contents)).toEqual({
    ok: false,
    error:
