...
const segment = (durations: number[], id = 'segment') => {
  let time = 0;
  return { id, samples: [0, ...durations].map((duration, index) => {
    time += duration;
    return { id: `${id}-${index}`, latitudeDegrees: 0, longitudeDegrees: index / 10000,
...
it('identifies isolated gaps and exact run edges without using speed', () => {
  const track = segment([600, ...Array(30).fill(10), 600, ...Array(30).fill(10), 600]);
  const points = analyseMeasurements([track]).points;
  expect(findRecordingGaps(points)).toEqual([1, 32, 63]);
  expect(findRecordingGaps(points.map(point => { return { ...point, speedMetresPerSecond: 0 }; }))).toEqual([1, 32, 63]);
});
...
])('preserves normal sparse sampling, ambiguous edges and strict boundaries (%#)', durations => {
  expect(findRecordingGaps(analyseMeasurements([segment(durations)]).points)).toEqual([]);
});
...
it('does not borrow context across segments or unusable timestamps', () => {
  ...
  expect(findRecordingGaps(analyseMeasurements([first, second]).points)).toEqual([]);
