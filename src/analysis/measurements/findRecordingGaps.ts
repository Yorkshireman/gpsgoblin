...
export const findRecordingGaps = (points: readonly MeasurementPoint[]) => {
  ...
  while (start < points.length) {
    if (points[start].intervalSeconds === null) {
      start += 1;
      continue;
    }
    let end = start + 1;
    while (
      ...
      points[end].segmentId === points[start].segmentId &&
      points[end].intervalSeconds !== null
    )
      end += 1;
    for (let index = start; index < end; index += 1) {
      ...
      const sides = [
        points.slice(Math.max(start, index - 20), index),
        points.slice(index + 1, Math.min(end, index + 21))
      ].filter((side) => {
      ...
      const reference = Math.max(
        ...sides.map((side) => {
          return median(
            side.map((point) => {
              return point.intervalSeconds ?? 0;
            })
          );
        })
      );
