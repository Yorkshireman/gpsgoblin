...
export const findStopCandidates = (
  ...
  let queues: number[][] = Array.from({ length: 6 }, () => {
    return [];
  });
  ...
  const flush = () => {
    ...
    if (missing) uncertainty.push('Elevation evidence is incomplete.');
      ...
      uncertainty.push('Elevation changes may be climbing or sensor drift.');
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    const { latitudeDegrees: latitude, longitudeDegrees: longitude } =
      point.sample;
    const valid =
    ...
    const continuous =
      ...
      previousValid &&
      index > 0 &&
      point.segmentId === points[index - 1].segmentId &&
      seconds !== null &&
      seconds > 0 &&
      seconds <= 10 &&
      !point.recordingGap;
    ...
    for (let queueIndex = 0; queueIndex < 6; queueIndex += 1) {
      ...
      while (
        queue.length > heads[queueIndex] &&
        coordinates[queue[queue.length - 1] * 3 + dimension] * sign >=
          coordinates[index * 3 + dimension] * sign
      )
        queue.pop();
      queue.push(index);
    }
    const extent = () => {
      return Math.hypot(
        ...[0, 1, 2].map((dimension) => {
          return (
            coordinates[
              queues[dimension * 2 + 1][heads[dimension * 2 + 1]] * 3 +
                dimension
            ] -
            coordinates[
              queues[dimension * 2][heads[dimension * 2]] * 3 + dimension
            ]
          );
        })
      );
    ...
    while (extent() > 10 && left < index) {
      left += 1;
      for (let queueIndex = 0; queueIndex < 6; queueIndex += 1) {
        while (queues[queueIndex][heads[queueIndex]] < left)
          heads[queueIndex] += 1;
      }
    }
    ...
    if (
      !best ||
      times[index] - times[left] > times[best.end] - times[best.start]
    )
      best = { start: left, end: index };
  }
  flush();
