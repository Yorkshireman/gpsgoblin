...
export const suggestPaceMaximum = (display: Float64Array, metrics: Float64Array, start: number, metresPerDistance: number) => {
  ...
  for (let index = 1; index < display.length / 3; index += 1) {
    ...
    const distance = metrics[(start + index) * 4] - metrics[(start + index - 1) * 4];
    if (!Number.isFinite(pace) || pace <= 0 || !Number.isFinite(distance) || distance <= 0) continue;
    values.push({ pace, distance });
    total += distance;
  }
  if (!values.length || !Number.isFinite(total)) return undefined;
  values.sort((left, right) => { return left.pace - right.pace; });
  let covered = 0;
