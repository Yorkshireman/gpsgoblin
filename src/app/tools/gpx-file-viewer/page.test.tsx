...
describe('GPX file viewer', () => {
  ...
  it('shows measurements, changes units and lets a keyboard-accessible point selector inspect source values', async () => {
    ...
    expect(screen.getByRole('slider', { name: 'Smoothing' })).toHaveValue('30');
    ...
    expect(
      screen.getByText(
        'Select a point on the chart to see its location on the map. Gaps show where measurements are missing.'
      )
    ).toBeVisible();
