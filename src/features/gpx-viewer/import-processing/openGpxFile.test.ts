...
jest.mock('./createImportWorker', () => ({
  createImportWorker: () => mockWorker
}));
...
it('allows a long import to finish instead of imposing a processing deadline', async () => {
  ...
  mockWorker.onmessage?.(
    new MessageEvent('message', {
      data: { requestId, result: { ok: false, error: 'Completed processing' } }
    })
  );
...
it.each(['error', 'messageerror'] as const)(
  'recovers from a worker %s with a useful recovery result',
  async (event) => {
    const pending = openGpxFile(
      new File(['<gpx/>'], 'route.gpx'),
      new AbortController().signal
  ...
  lateMessage?.(
    new MessageEvent('message', {
      data: {
        ...
        result: { ok: false, error: 'Late result' }
      }
    })
  );
  await rejection;
  ...
  mockWorker.onmessage?.(
    new MessageEvent('message', {
      data: {
        ...
        result: { ok: false, error: 'Wrong request' }
      }
    })
  );
  mockWorker.onmessage?.(
    new MessageEvent('message', {
      data: { requestId, result: { ok: false, error: 'Correct request' } }
    })
  );
  await expect(pending).resolves.toEqual({
    ok: false,
...
it('sends a large file to the worker and lets the user cancel it', async () => {
  const controller = new AbortController();
  const pending = openGpxFile(
    new File(['x'.repeat(3 * 1024 * 1024)], 'large.gpx'),
    controller.signal
  );
  const rejection = expect(pending).rejects.toMatchObject({
    name: 'AbortError'
...
it('releases the worker and returns a recovery error if sending the file fails', async () => {
  ...
  await expect(
    openGpxFile(new File(['<gpx/>'], 'route.gpx'), new AbortController().signal)
  ).resolves.toEqual({
    ok: false,
    error:
      'The file could not be processed. Try again or choose another GPX file.'
...
it('coalesces queued settings and ignores cancelled and unrelated view replies', async () => {
  ...
  const firstRejected = expect(firstResult).rejects.toMatchObject({
    name: 'AbortError'
  });
  const secondResult = opened.measurements.prepareView(
    { ...settings, smoothingSeconds: 120 },
    second.signal
  );
  ...
  first.abort();
  second.abort();
  const latestResult = opened.measurements.prepareView(
    { ...settings, smoothingSeconds: 600 },
    new AbortController().signal
  );
  const latestRejected = expect(latestResult).rejects.toThrow(
  ...
  mockWorker.onmessage?.(
    new MessageEvent('message', {
      data: { type: 'view', requestId: -1, ok: false, error: 'Unrelated' }
    })
  );
  ...
  mockWorker.onmessage?.(
    new MessageEvent('message', {
      data: {
        ...
        ok: false,
        error: 'Recoverable view failure'
      }
    })
  );
