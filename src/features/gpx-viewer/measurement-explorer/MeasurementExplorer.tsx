  ...
  const segments = useMemo(() => {
    return track
      ? segment
        ? [segment]
        : track.segments
      : [{ id: route.id, samples: route.points }];
  }, [track, segment, route]);
  const settings = useMemo(() => {
    return {
      entityId: track ? track.id : route.id,
  ...
  const options = (
    <>
      <StopOptions
        ...
        axis={useTime ? 'time' : 'distance'}
        onAxisChange={setAxis}
        hasStops={analysis.recorded && analysis.stops.candidates.length > 0}
        timeAvailable={prepared.snapshot.timeAvailable}
      />
      {analysis.recorded && analysis.stops.candidates.length > 0 ? (
        <Box pt={2}>
          <StopReview
            evidence={analysis.stops}
    ...
    !hasTimedMotion && hasElevation
      ...
      : activeChart === 'elevation' && !hasElevation
        ...
        : activeChart === 'elevation'
          ...
          : motion;
  const paceMaximum =
    paceMaximumPerKm === undefined
      ? undefined
      : paceMaximumPerKm * (labels.metresPerDistance / 1000);
  const axisMaximum =
    chart === 'pace'
      ? paceRange === 'suggested'
        ? prepared.snapshot.suggestedPaceMaximum
        : paceRange === 'custom'
          ? paceMaximum
          : undefined
      : undefined;
  ...
  const motionChart = (
    <MeasurementChart
      ...
      reference={
        ...
        basis.averageSpeedMetresPerSecond !== null &&
        (motion === 'speed' || basis.averageSpeedMetresPerSecond > 0)
          ? {
              value:
                motion === 'speed'
              ...
              onModeChange={(mode) => {
                if (mode === 'custom' && !hasChosenCustomMaximum) {
                  ...
                  setPaceMaximumPerKm(
                    suggested === undefined
                      ? undefined
                      : suggested / (labels.metresPerDistance / 1000)
                  );
          ...
          {chart === 'elevation' ? (
            ...
            <>
              ...
              {hasMotion ? (
                <Stack gap={2} mt={2}>
                  {motionChart}
                  {selectionTip ? <Box mb={2}>{selectionTip}</Box> : null}
                  <MotionControls
                    pending={prepared.pending}
                ...
                <Text>
                  {isMoving
                    ? 'Nothing is left to show with these stops excluded. Choose Include stops to see the chart again.'
