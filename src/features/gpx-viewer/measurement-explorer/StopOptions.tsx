...
export const StopOptions = ({ mode, onModeChange, axis, onAxisChange, recorded, timeAvailable }: StopOptionsProps) => {
  return <Grid templateColumns='repeat(2, minmax(0, 1fr))' gap={2} pt={2}>
    <Field.Root disabled={!recorded}>
      <Field.Label>Stop calculation</Field.Label>
      <NativeSelect.Root size='sm'>
        <NativeSelect.Field value={mode} onChange={event => {
          ...
          <option value='exclude'>Exclude detected stops</option>
        </NativeSelect.Field>
        <NativeSelect.Indicator />
      </NativeSelect.Root>
    </Field.Root>
    <Field.Root>
      <Field.Label>Horizontal axis</Field.Label>
      <NativeSelect.Root size='sm'>
    ...
    {!timeAvailable ? <Text gridColumn='1 / -1' fontSize='xs'>Time axis unavailable for this calculation. Distance and source points remain available.</Text> : null}
  </Grid>;
};
