...
export const GapMarkers = ({
  ...
  for (const point of data) {
    if ((!point.recordingGap && !point.possibleStop) || !point.sampleId)
      continue;
    ...
    if (previous && x - previous.x < 44) {
      previous.ids.push(point.sampleId);
      previous.hasStops ||= Boolean(point.possibleStop);
    } else
      groups.push({
        x,
        ids: [point.sampleId],
        hasStops: Boolean(point.possibleStop)
      });
  }
  return (
    <ZIndexLayer zIndex={DefaultZIndexes.label + 3}>
            ...
            <Tooltip.Root openDelay={250} closeDelay={0}>
              <Tooltip.Trigger asChild>
                <IconButton
                  ...
                  onClick={(event) => {
                    onSelect(
                      group.ids.length === 1 && selectedIndex === 0
                        ? undefined
                        : group.ids[(selectedIndex + 1) % group.ids.length],
                      event.currentTarget
                    );
                  }}
                >
                  <svg
                    ...
                    fill="none"
                    stroke="currentColor"
                    strokeWidth={selectedIndex >= 0 ? 2 : 1.5}
                  >
                    {selectedIndex >= 0 ? (
                      <circle
                        cx="12"
                    ...
                    {group.hasStops ? (
                      ...
                      <path d="M4 12h4m8 0h4M10 7l-2 10m8-10-2 10" />
                    )}
                  </svg>
                </IconButton>
              </Tooltip.Trigger>
