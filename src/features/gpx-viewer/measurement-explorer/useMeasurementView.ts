...
type Analysis = ReturnType<typeof analyseMeasurements> &
  Pick<PackedMeasurementAnalysis, 'stops' | 'recorded'>;
const nullable = (value: number) => {
  return Number.isNaN(value) ? null : value;
};

const unpackAnalysis = (
  packed: PackedMeasurementAnalysis,
  segments: readonly TrackSegment[]
): Analysis => {
  const points: MeasurementPoint[] = [];
  const issues = new Map(
    packed.timeIssues.map((item) => {
      return [item.index, item.issue];
    })
  );
  for (const segment of segments) {
    for (const sample of segment.samples) {
      ...
      points.push({
        ...
        distanceMetres: packed.metrics[index * 4],
        elevationMetres: nullable(packed.metrics[index * 4 + 1]),
        speedMetresPerSecond: nullable(packed.metrics[index * 4 + 2]),
        intervalSeconds: nullable(packed.metrics[index * 4 + 3]),
        timeIssue: issues.get(index)
      });
    }
  }
  for (const index of packed.recordingGaps ?? []) {
    ...
    if (point && previous && point.intervalSeconds !== null)
      points[index] = {
        ...point,
        recordingGap: {
          seconds: point.intervalSeconds,
          distanceMetres: point.distanceMetres - previous.distanceMetres,
          startSampleId: previous.sample.id
        }
      };
  }
  return {
...
const unpackDisplay = (
  ...
  for (const range of packed.excludedRanges)
    excluded.fill(1, range.startIndex + 1, range.endIndex + 1);
  const starts = new Set(
    packed.excludedRanges.map((range) => {
      return range.startIndex;
    })
  );
  const ends = new Set(
    stops.candidates.map((candidate) => {
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    if (index > 0 && point.segmentId !== points[index - 1].segmentId) {
      data.push({
        sampleId: null,
        distance,
        timeMinutes: packed.timeSeconds[index] / 60,
        motion: null,
        elevation: null
      });
    }
    ...
    data.push({
      ...
      elevation,
      timeMinutes: packed.timeSeconds[index] / 60,
      excludedStop: excluded[index] === 1,
      stopBoundary: starts.has(index),
      possibleStop: ends.has(index),
      recordingGap: Boolean(point.recordingGap),
      zeroSpeed: !point.recordingGap && point.speedMetresPerSecond === 0
    });
  }
...
export const useMeasurementView = (
  ...
  const receivedAnalysis = useRef<
    { scope: typeof scope; analysis: Analysis } | undefined
  >(undefined);
  ...
  const [result, setResult] = useState<{
    ...
    basis: PackedMeasurementView['basis'];
  ...
  useEffect(() => {
    ...
    session
      .prepareView({ ...settings, includeAnalysis }, controller.signal)
      .then((packed) => {
        if (controller.signal.aborted) return;
        let analysis =
          receivedAnalysis.current?.scope === scope
            ? receivedAnalysis.current.analysis
            : undefined;
        if (!analysis) {
          if (!packed.analysis)
            throw new Error('Measurement details were unavailable. Try again.');
        ...
        setResult({
          ...
          analysis,
          basis: packed.basis,
          timeAvailable: packed.timeAvailable,
          suggestedPaceMaximum: packed.suggestedPaceMaximum,
          ...unpackDisplay(packed, analysis.points, analysis.stops)
        });
      })
      .catch((error) => {
        if (controller.signal.aborted) return;
        setFailure({
          ...
          message:
            error instanceof Error
              ? error.message
              : 'Measurements could not be updated. Try again.'
        });
      });
  ...
  const error =
    failure?.scope === scope && failure.settings === settings
      ? failure.message
      : undefined;
  return {
    snapshot,
    error,
    pending: snapshot?.settings !== settings && !error,
    retry: () => {
