...
const profileDirectory =
  process.env.GPX_PROFILE_DIR || '/tmp/gpsgoblin-memory-profiles';
mkdirSync(profileDirectory, { recursive: true });
const counts = (process.env.GPX_MEMORY_POINTS || '250000')
  .split(',')
  .map(Number);
const variants = (process.env.GPX_MEMORY_VARIANTS || 'plain').split(',');
const runs = Number(process.env.GPX_MEMORY_RUNS || 3);
const viewport = process.env.GPX_PHONE
  ? { width: 390, height: 844 }
  : { width: 1280, height: 720 };
const baseUrl = process.env.GPX_PROFILE_URL || 'http://127.0.0.1:4173';
if (
  !counts.every((count) => Number.isSafeInteger(count) && count > 1) ||
  !Number.isSafeInteger(runs) ||
  runs < 1 ||
  !variants.every((variant) =>
    ['plain', 'extensions', 'fragmented'].includes(variant)
  )
) {
...
const cases = [
  {
    name: 'sanitised-recording',
    path: resolve('tests/fixtures/gpx/strava-recording.gpx')
  }
];
if (process.env.GPX_VERIFY_FILE) {
  cases.unshift({
    name: 'private-recording',
    path: process.env.GPX_VERIFY_FILE
  });
}
...
console.log(
  JSON.stringify({
    ...
    loadedObservationTailMs: 1000,
    interpretation:
      'Sampled sum of RSS for a fresh browser process and its descendants; includes shared pages and overhead, excludes Node and other applications, may miss short-lived peaks. Paired WebGL-disabled trials estimate incremental map work, not an exact isolated map cost.'
  })
);

for (const scenario of cases.filter(
  (scenario) =>
    !process.env.GPX_MEMORY_CASE ||
    scenario.name.includes(process.env.GPX_MEMORY_CASE)
)) {
        ...
        pendingSample = (async () => {
          try {
            samples.push({
              ...
              ...(await browserRss(server.process().pid))
            });
          } catch (error) {
            sampleFailures.push(
              error instanceof Error ? error.name : 'Unknown sampling error'
            );
          } finally {
      ...
      try {
        ...
        await page.addInitScript(
          ({ mapEnabled }) => {
            ...
            new PerformanceObserver((list) => {
              window.memoryProfile.tasks.push(
        ...
        try {
          await page
            ...
            .setInputFiles(scenario.path);
          const failure = page.getByText('Unable to open GPX file', {
            exact: true
          });
          ...
          if (await failure.isVisible()) {
            ...
            await afterPaint(page);
            chartPaintMs = await page.evaluate(() =>
              Math.round(performance.now() - window.memoryProfile.input)
            );
            sourcePoints = Number(await position.getAttribute('max')) + 1;
            const selectionStart = performance.now();
            await position.press('End');
            await page
              .getByLabel('Selected measurement', { exact: true })
              .waitFor();
            await afterPaint(page);
            ...
            if (mapEnabled) {
              ...
              const marker = page.getByRole('img', {
                name: /^Selected map position:/
              });
              const unavailable = page.getByText('Map unavailable', {
                exact: true
              });
              await marker.or(unavailable).first().waitFor();
              mapOutcome = (await marker.isVisible())
                ? 'selected-position-ready'
                : 'unavailable';
              await afterPaint(page);
          ...
          outcome =
            error instanceof Error ? error.name : 'Unknown browser failure';
        }
        ...
        const tasks =
          profile?.tasks.filter((task) => task.start >= profile.input) ?? [];
        const peak = samples.reduce(
          (peak, next) => (next.rssBytes > peak.rssBytes ? next : peak),
          { rssBytes: 0, processes: 0 }
        );
        console.log(
          JSON.stringify({
            kind: 'trial',
            scenario: scenario.name,
            bytes: statSync(scenario.path).size,
            ...
            selectionPaintMs,
            mapSelectionReadyMs,
            maximumMainThreadTaskMs: Math.round(
              tasks.reduce((max, task) => Math.max(max, task.ms), 0)
            ),
            baselineRssBytes,
            sampledPeakRssBytes: peak.rssBytes,
            sampledPeakAboveBaselineBytes:
              baselineRssBytes === undefined
                ? undefined
                : peak.rssBytes - baselineRssBytes,
            sampledFinalRssBytes: samples.at(-1)?.rssBytes,
            processCountAtPeak: peak.processes,
        ...
        console.log(
          JSON.stringify({
            ...
            mapEnabled,
            viewport,
            outcome: 'profiling-setup-failed',
            error: error instanceof Error ? error.name : 'Unknown setup error',
            sampleFailures
          })
        );
      } finally {
