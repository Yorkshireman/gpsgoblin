...
const outputDirectory =
  process.env.GPX_PROFILE_DIR || '/tmp/gpsgoblin-profiles';
mkdirSync(outputDirectory, { recursive: true });
const counts = (process.env.GPX_BENCHMARK_POINTS || '10000,50000,100000,250000')
  .split(',')
  .map(Number);
...
try {
  for (const cpuRate of (process.env.GPX_CPU_RATES || '1,4')
    .split(',')
    .map(Number)) {
    for (const scenario of cases.filter((item) => {
      return (
        !process.env.GPX_BENCHMARK_CASE ||
        item.name.includes(process.env.GPX_BENCHMARK_CASE)
      );
    })) {
      for (
        let run = 1;
        run <= Number(process.env.GPX_BENCHMARK_RUNS || 3);
        run++
      ) {
        ...
        await page.addInitScript(() => {
          ...
          const mark = (name) => {
            window.importProfile.events[name] = performance.now();
          };
          document.addEventListener(
            ...
            (event) => {
              if (
                ...
                !window.importProfile.events.input
              )
                mark('input');
            },
            true
          ...
          new PerformanceObserver((list) => {
            window.importProfile.tasks.push(
        ...
        try {
          ...
          await page.evaluate(() => {
            window.importProfile.events.painted = performance.now();
          });
        } catch {
        ...
        writeFileSync(
          resolve(
            outputDirectory,
            `${scenario.name}-${cpuRate}-${run}.cpuprofile`
          ),
        ...
        if (outcome === 'loaded') {
          ...
          await page
            .getByRole('slider', { name: 'Smoothing' })
            .press('ArrowRight');
          await page.waitForFunction(
            () => {
              return (
                document
                  .querySelector('[aria-label="Measurement chart"]')
        ...
        const metric = (response, name) => {
          return (
            response.metrics.find((item) => item.name === name)?.value || 0
          );
        };
        const { input, workerCreated, workerDelivered, painted } =
          imported.events;
        const tasks = imported.tasks.filter((task) => {
          return task.start >= input;
        });
        const data = {
          scenario: scenario.name,
          bytes: readFileSync(scenario.path).length,
          cpuRate,
          run,
          viewport,
          outcome,
          workerToPaintMs: Math.round(painted - workerCreated),
          workerRoundTripMs: Math.round(workerDelivered - workerCreated),
          afterWorkerMs: Math.round(painted - workerDelivered),
          selectionMs,
          chartClickMs,
          smoothingMs,
          sourcePoints,
          drawnVertices,
          maxMainThreadTaskMs: Math.round(
            tasks.reduce((max, task) => Math.max(max, task.ms), 0)
          ),
          mainThreadScriptMs: Math.round(
            1000 *
          ...
          layoutMs: Math.round(
            1000 *
              (metric(after, 'LayoutDuration') -
                metric(before, 'LayoutDuration'))
          ),
          pageHeapUsedMiB: Math.round(
            metric(after, 'JSHeapUsedSize') / 1024 ** 2
          )
        };
