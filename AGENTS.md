...
Documentation-only work is exempt from the branch and PR requirement. During an authorised `/agent` request, changes that affect only documentation or agent instruction files may be committed directly to `master`, or committed on another branch and merged locally into `master`, then pushed to GitHub without a PR. Before committing or merging, inspect the complete diff and use the normal PR workflow if it contains application code, tests, dependency or runtime configuration, generated runtime assets or any other executable behaviour. Run the applicable documentation and repository checks, and report the commit and push results.
